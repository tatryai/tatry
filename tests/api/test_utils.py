...
def test_get_usage_default(client, mock_responses):
    mock_responses.add(
        ...
        json={
            "status": "success",
            "data": {
                "time_range": {"month": "2024-02"},
                "usage": {
                    "queries": {
                        "total": 1000,
                        "by_source": {"wikipedia": 500, "academic_journals": 500},
                    },
                    "documents": {
                        "total": 5000,
                        "by_source": {"wikipedia": 2500, "academic_journals": 2500},
                    },
                },
            },
        },
        ...
        json={
            ...
            "data": {
                ...
                "usage": {
                    "queries": {
                        "total": 500,
                        "by_source": {"wikipedia": 250, "academic_journals": 250},
                    },
                    "documents": {
                        "total": 2500,
                        "by_source": {"wikipedia": 1250, "academic_journals": 1250},
                    },
                },
            },
        },
...
def test_submit_feedback_with_metadata(client, mock_responses):
    ...
    metadata = {"browser": "Chrome", "version": "120.0", "os": "Windows"}
    ...
    mock_responses.add(
        ...
        match=[
            responses.matchers.json_params_matcher(
                {"type": "bug", "description": "Test feedback", "metadata": metadata}
            )
        ],
        json={
            "status": "success",
    ...
    response = client.utils.submit_feedback(
        feedback_type="bug", description="Test feedback", metadata=metadata
    )
...
def test_submit_feedback_validation_error(client, mock_responses):
    mock_responses.add(
        mock_responses.POST,
        f"{API_URL}/v1/feedback",
        json={"error": "Validation error", "details": "Invalid feedback type"},
        status=400,
    )

    with pytest.raises(TatryAPIError) as exc:
        client.utils.submit_feedback(feedback_type="invalid", description="")
    ...
    mock_responses.add(
        ...
        json={
            ...
            "data": {
                "status": "healthy",
                "version": "1.0.0",
                "timestamp": "2025-02-18T15:59:27Z",
            },
        },
        status=200,
    )
